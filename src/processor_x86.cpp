...
static inline std::string host_cpu_name()
{
...
static TargetData<feature_sz> arg_target_data(const TargetArg<feature_sz> &arg, bool require_host)
{
    ...
    enable_depends(res.features, Feature::deps, sizeof(Feature::deps) / sizeof(FeatureDep));
    for (size_t i = 0; i < feature_sz; i++)
    ...
    disable_depends(res.features, Feature::deps, sizeof(Feature::deps) / sizeof(FeatureDep));
    return res;
}
